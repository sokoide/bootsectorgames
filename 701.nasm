...
; 0x0000=f9ff is vislble VRAM
base:           equ 0xfc80

; global variables
shots:          equ base+0x00
old_time:       equ base+0x0c
level:          equ base+0x10   ; current level
lives:          equ base+0x11   ; current lives
sprites:        equ base+0x12   ; sprite table SIRITE_SIZE * (1+55)

SHIP_ROW:       equ 0x5c*OFFSET_X       ; Row of spaceship -> row 92. 1 row has 2 graphic lines
X_WIDTH:        equ 0x0140      ; 320. w-width
OFFSET_X:       equ X_WIDTH*2   ; 1 row has 2 screen lines
SPRITE_SIZE:    equ 4           ; 1 sprite has 4 byte data. 1 for color, 1 for state, 2 for X, Y

; constants
SPACESHIP_COLOR:            equ 0x1c        ; Must be < 0x20
...
START_COLOR:    equ ((sprites+SPRITE_SIZE-(shots+2))/SPRITE_SIZE+0x20)
    ...
    %else
        org 0x7c00
    %endif
        mov ax,0x0013   ; 320x200x256 colors
        int 0x10
        cld             ; reset direction
